...
<section>
    ...
    <form method="post">
        ...
        <input type="date" id="amt" name="amt" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO transaction(transactionid, userid, propertyid, date, amount) VALUES (?, ? ,?, ?, ?)");
    $stmt->bind_param("sssss", $tid, $uid, $prid, $dt, $amt);
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of transaction</title>
    <style>
        table {
            width: 100%;
...
<body>
    ...
    <table border="5">
        ...
        // Prepare SQL query to retrieve all transaction
        $sql = "SELECT * FROM transaction";
        $result = $connection->query($sql);

        // Check if there are any transaction
        if ($result->num_rows > 0) {
            // Output data for each row
            while ($row = $result->fetch_assoc()) {
                $tid = $row['transactionid']; // Fetch the transactionid
                echo "<tr>
                    ...
                    <td>" . $row['date'] . "</td>
                    <td>" . $row['amount'] . "</td>
                    <td><a style='padding:4px' href='delete_transaction.php?transactionid=$tid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_transaction.php?transactionid=$tid'>Update</a></td> 
                </tr>";
            }
        } else {
