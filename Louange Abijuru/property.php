...
<section>
    ...
    <form method="post">
        ...
        <input type="number" id="adr" name="adr"><br><br>

        <label for="ty">type:</label>
        <input type="text" id="ty" name="ty" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO property(propertyid, address, type, price) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $prid, $adr, $ty, $pr);
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of property</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of property</h2></center>
    <table border="5">
        <tr>
            <th>propertyid</th>
            <th>address</th>
        ...
        // Check if there are any property
        if ($result->num_rows > 0) {
            // Output data for each row
            while ($row = $result->fetch_assoc()) {
                $prid = $row['propertyid']; // Fetch the propertyid
                echo "<tr>
                    <td>" . $row['propertyid'] . "</td>
                    <td>" . $row['address'] . "</td>
                    <td>" . $row['type'] . "</td>
                    <td>" . $row['price'] . "</td>
                    <td><a style='padding:4px' href='delete_property.php?propertyid=$prid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_property.php?propertyid=$prid'>Update</a></td> 
                </tr>";
            }
        } else {
